...
fn find_sidecar_dev() -> Result<PathBuf, String> {
    ...
    let dev_path = PathBuf::from(manifest_dir)
        .join("binaries")
        .join(format!("save_pandetta-{}", target));
    if dev_path.exists() {
        return Ok(dev_path);
    }

    // Fallback: nome semplice
    let simple = PathBuf::from(manifest_dir).join("binaries").join("save_pandetta");
    if simple.exists() {
        return Ok(simple);
    }

    Err(format!(
        "Sidecar non trovato in src-tauri/binaries/. Cercato: save_pandetta-{} e save_pandetta",
        target
    ))
}
